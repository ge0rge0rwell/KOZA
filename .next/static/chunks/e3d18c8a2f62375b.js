(globalThis.TURBOPACK||(globalThis.TURBOPACK=[])).push(["object"==typeof document?document.currentScript:void 0,11187,e=>{"use strict";var t=e.i(53458);let i=`You are OMNIVERSAL NARRATIVE SINGULARITY CORE.
...
`,a=`You are a "Bullying Coping" guide. You take the bullying or traumatic experience experienced by the user and turn it into at least 10 pages of long, rich, morale-boosting and supportive story that turns it into a process of "Motivating the User, Ensuring They Overcome Difficulties".
...
Write nothing besides JSON.`,n=`You are a story editor. You take an existing story and the user's feedback and update the story according to this feedback.
...
Write nothing besides JSON.`,r=`You are an interactive metamorphosis designer. You transform the user's experience into a 3-level "Inner Strength Labyrinth" game.
...
Write nothing besides JSON.`,o=`You are a creative naming expert. Create a metaphorical, short, and impressive title suitable for the "KOZA" universe, according to the given story or game content and context.
...
Context/Content: `,s=new Map,l=e=>{try{let t=e.replace(/```json/g,"").replace(/```/g,"").trim(),i=t.indexOf("["),a=t.indexOf("{"),n=-1,r=-1;return -1!==i&&(-1===a||i<a)?(n=i,r=t.lastIndexOf("]")):-1!==a&&(n=a,r=t.lastIndexOf("}")),-1!==n&&-1!==r&&(t=t.substring(n,r+1)),t}catch{return e}},c=e=>new Promise(t=>setTimeout(t,e)),u=async(e,a,n=3)=>{let r,o=t.API_CONFIG.OPENROUTER_API_KEY,u=window.location.origin,d=`${e.substring(0,50)}_${a.substring(0,100)}`,g=s.get(d);if(g&&Date.now()-g.timestamp<3e5)return console.log("📦 Using cached response"),g.data;for(let t=0;t<n;t++)try{let t=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${o}`,"HTTP-Referer":u,"X-Title":"KOZA App"},body:JSON.stringify({model:"google/gemma-3-27b-it",messages:[{role:"system",content:i},{role:"user",content:`${e}

Kullanıcının deneyimi: ${a}`}],temperature:.8,max_tokens:8192,response_format:{type:"json_object"}})});if(!t.ok){let e=await t.text();throw Error(`API error ${t.status}: ${e}`)}let n=(await t.json()).choices[0].message.content,r=JSON.parse(l(n));return s.set(d,{data:r,timestamp:Date.now()}),r}catch(e){if(r=e,console.error(`Attempt ${t+1} failed:`,e.message),t<n-1){let e=Math.min(1e3*Math.pow(2,t),5e3);console.log(`Retrying in ${e}ms...`),await c(e)}}throw Error(`Failed after ${n} attempts: ${r.message}`)},d=async e=>{if(!e||e.trim().length<10)throw Error("Lütfen en az 10 karakter uzunluğunda bir hikaye girin");return u(a,e)},g=async(e,t)=>{if(!t||t.trim().length<5)throw Error("Lütfen daha detaylı bir geri bildirim girin");return u(n.replace("{{EXISTING_STORY}}",JSON.stringify(e)).replace("{{USER_FEEDBACK}}",t),t)},E=async e=>{if(!e||e.trim().length<10)throw Error("Lütfen en az 10 karakter uzunluğunda bir deneyim girin");return u(r,e)},m=async e=>{try{let t=o+`

Yanıtı şu JSON formatında ver: { "title": "Oluşturulan Başlık" }`;return(await u(t,e)).title}catch{return console.error("Naming failed"),"Dönüşüm Hikayesi"}};setInterval(()=>{let e=Date.now();for(let[t,i]of s.entries())e-i.timestamp>3e5&&s.delete(t)},3e5),e.s(["generateContentName",0,m,"generateGame",0,E,"generateStorybook",0,d,"refineStorybook",0,g],11187)},80763,54766,e=>{"use strict";let t=["self harm","suicide","want to die","kill myself","stab","with gun","hang myself","poison","kill someone","want to hurt"],i=e=>{if(!e||"string"!=typeof e)return{isCrisis:!1};let i=e.toLowerCase().trim();return t.filter(e=>i.includes(e)).length>0?{isCrisis:!0,message:"This platform is for educational purposes. If you feel in danger to yourself or others, please seek professional help immediately or call emergency services (e.g., 911)."}:{isCrisis:!1}};e.s(["SAFETY_DISCLAIMER",0,"KOZA is an educational tool and does not replace professional psychological support.","detectCrisis",0,i],54766);var a=e.i(11187);let n=e=>{let t=[];if(!e||"string"!=typeof e)return t.push("Geçerli bir metin girmelisiniz"),{isValid:!1,errors:t};let i=e.trim();return i.length<10&&t.push("Lütfen en az 10 karakter girin"),i.length>5e3&&t.push("Metin çok uzun (maksimum 5000 karakter)"),{isValid:0===t.length,errors:t,sanitized:i}},r=new Map,o=new Map,s={STATES:{IDLE:"IDLE",VALIDATING:"VALIDATING",SAFETY_CHECK:"SAFETY_CHECK",AI_COORDINATION:"AI_COORDINATION",MAPPING:"MAPPING",COMPLETED:"COMPLETED",FAILED:"FAILED"},resolveMetadata:(e,t,i)=>({type:e,title:t||("story"===e?"Dönüşüm Hikayesi":"Dönüşüm Oyunu"),userInput:i,reflectionQuestion:"Bu hikaye sana kendi gücün hakkında ne söylüyor?",growthLesson:"Zorluklar gelişimin habercisidir.",createdAt:new Date().toISOString()}),processNarrativeRequest:async(e,t="story")=>{let l=`${t}:${e.trim().toLowerCase()}`;if(o.has(l))return console.log("🚀 Optimization: Serving from Narrative Cache for:",l),o.get(l);if(r.has(l))return console.warn("⚡ Optimization: Deduplicating concurrent request for:",l),r.get(l);let c=(async()=>{try{let r=n(e);if(!r.isValid)throw Error(r.errors[0]);let c=i(r.sanitized);if(c.isCrisis)return{isSafetyTriggered:!0,message:c.message,redirect:"SAFETY_RESOURCES"};let[u,d]=await Promise.all(["story"===t?(0,a.generateStorybook)(r.sanitized):(0,a.generateGame)(r.sanitized),(0,a.generateContentName)(r.sanitized)]),g={isSafetyTriggered:!1,data:{...s.resolveMetadata(t,d,r.sanitized),pages:"story"===t?u.pages:void 0,levels:"game"===t?u.levels:void 0,themeColor:u.themeColor||"#9333EA",visualMood:u.visualMood||"Magical Shimmer"}};if(o.size>=50){let e=o.keys().next().value;o.delete(e)}return o.set(l,g),g}catch(e){throw console.error("Domain Lifecycle Error:",e),Error(`Optimizasyon Katmanı Hatası: ${e.message}`)}finally{r.delete(l)}})();return r.set(l,c),c},processRefinementRequest:async(t,a)=>{let o=`refine:${t.id}:${a.trim().toLowerCase()}`;if(r.has(o))return r.get(o);let l=(async()=>{try{let r=n(a);if(!r.isValid)throw Error(r.errors[0]);let o=i(r.sanitized);if(o.isCrisis)return{isSafetyTriggered:!0,message:o.message};let[l,c]=await Promise.all([e.A(1110).then(e=>e.refineStorybook(t,r.sanitized)),e.A(1110).then(e=>e.generateContentName(r.sanitized))]);return{isSafetyTriggered:!1,data:{id:t.id,...s.resolveMetadata("story",c,t.userInput+" | Refinement: "+r.sanitized),pages:l.pages,themeColor:l.themeColor||t.themeColor,visualMood:l.visualMood||t.visualMood,refinedAt:new Date().toISOString()}}}catch(e){throw console.error("Refinement Domain Error:",e),Error(`Hikaye d\xfczenleme hatası: ${e.message}`)}finally{r.delete(o)}})();return r.set(o,l),l}};e.s(["NarrativeDomain",0,s],80763)},1110,e=>{e.v(e=>Promise.resolve().then(()=>e(11187)))}]);